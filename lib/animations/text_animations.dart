...
class _HomPageState extends State<HomPage>  with SingleTickerProviderStateMixin{
  ...
  void initState() {
    ...
    super.initState();
    _animatedContainer = AnimationController(vsync: this, duration: const Duration(milliseconds: 3000));
    _rotateAnimation = Tween(begin:  0.0, end: 1.0).animate(
      CurvedAnimation(parent: _animatedContainer, curve: const Interval(0.0,0.5,curve: Curves.bounceInOut)
      ),
    );

    _slideAnimation = Tween(begin:100.0, end:600.0).animate(
      CurvedAnimation(parent: _animatedContainer, curve: const Interval(0.5,1.0,curve: Curves.fastOutSlowIn)
      ),
    );
      ...
      body: AnimatedBuilder(
        ...
        builder: (ctx, ch) => Container(
          ...
          child: RotationTransition(
            ...
            child: Center(
              child: Text('DIYORBEK', style:  TextStyle(fontSize: 40,fontWeight: FontWeight.w700, color: Color.fromRGBO(0, 0, 128, _opaciyAnimation.value)),),
            ),
          ),
        ),
